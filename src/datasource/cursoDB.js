...
cursoDb.create = function (curso, funCallback) {
    var query = 'INSERT INTO cursos (idcurso,nombre,descripcion,estado) VALUES (?,?,?,?)'
    var dbParams = [curso.idcurso, curso.nombre, curso.descripcion, curso.estado];
 ...
 */
 cursoDb.update = function (idcurso, curso, funCallback) {
    var query = 'UPDATE cursos SET id = ? , nombre = ?, descripcion = ?, estado = ? WHERE idcurso = ?'
    var dbParams = [curso.idcurso, curso.nombre, curso.descripcion, curso.estado, idcurso];
    connection.query(query, dbParams, function (err, result, fields) {
        if (err) {
            funCallback({
                code:3,
